...
def load_model():
    try:
        ...
        st.write("Attempting to download the model...")
        model_response = requests.get(model_url)
        st.write(f"Model download response: {model_response.status_code}")
        if model_response.status_code == 200:
            st.write("Model downloaded successfully!")
            return joblib.load(io.BytesIO(model_response.content))
        else:
            st.error(f"Model download failed with status code {model_response.status_code}")
